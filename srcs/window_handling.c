...
/*   Updated: 2024/06/09 21:18:00 by tamehri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	cub->tex[0].img.__img = mlx_xpm_file_to_image(cub->mlx.__mlx, \
		cub->tex[0].file, &cub->tex[0].img.width, &cub->tex[0].img.width);
	if (!cub->tex[0].img.__img)
		(putendl_fd(ERR_MLX_IMG, 2), exit(EXIT_FAILURE));
	cub->tex[0].img.__addr = mlx_get_data_addr(cub->tex[0].img.__img, \
		&cub->tex[0].img.pixel_bits, &cub->tex[0].img.line_bytes, \
		&cub->tex[0].img.endian);
	if (!cub->tex[0].img.__addr)
